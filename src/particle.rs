...
use crate::{position_mod::PositionMod, rng::SplitMix64};
...
///
/// Read model (all in WaveTable t-space `[0, 1)`, see Architecture.md §4):
/// every sample the position advances by `playback_rate / capacity` (pitch
/// drift) and is pulled toward `onset + mod_offset` through a smoothed
...
impl Particle {
    ...
    ///
    /// All time-like values are in sample/sample-rate units; `smooth_ms` is
    /// the double-time-constant follower time for position smoothing.
    ...
    pub fn new(
        ...
        let decay_per_sample =
            1e-3f32.powf(1.0 / lifetime_samples as f32);
        Self {
    ...
    pub fn process(
        ...
        // Modulated & smoothed onset target.
        let offset = self.position_mod.next_offset(dt, sample_count, rng);
        let target = (self.onset + offset).clamp(0.0, 1.0);
        ...
        if self.attack_elapsed < self.attack_samples {
            ...
            self.gain = self.initial_gain
                * (self.attack_elapsed as f32 / self.attack_samples as f32);
        } else {
